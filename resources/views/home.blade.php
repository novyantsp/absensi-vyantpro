        ...
        @else
            @if ($present)
                @if ($present->keterangan == 'Alpha')
                    <div class="text-center">
                        @if (strtotime(date('H:i:s')) >= strtotime('15:00:00') && strtotime(date('H:i:s')) <= strtotime('21:00:00'))
                ...
                @else
                    <div class="text-center">
                        <p>
                            Check-in hari ini pukul : <b>{{ ($present->jam_masuk) }}</b>
                        </p>
                        @if ($present->jam_keluar)
                            <p>Check-out hari ini pukul : <b>{{ $present->jam_keluar }}</b></p>
                        @else
                            @if ((\Carbon\Carbon::parse($present->jam_masuk)->diffInHours(\Carbon\Carbon::parse(date('H:i:s')))) >= 2)
                                <p>Mau pulang ? jangan lupa check out dulu</p>
